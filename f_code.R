setwd("C:/Users/user/Desktop/analytics vidhya/big market data")

train<-read.csv(file="Train_UWu5bXk.csv", sep=",", header=T)
test<-read.csv(file="Test_u94Q5KV.csv", sep=",", header = T)
summary(train)
...
# library(dummies)
# total_1<-dummy.data.frame(total,names=c("Outlet_Type","Outlet_Size","Outlet_Location_Type"))
# 
# library(ggplot2)
# ggplot(total,aes(x=Item_Type,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# levels(total$Item_Type)[levels(total$Item_Type) %in% c("Starchy Foods","Breads","Breakfast")]<-"Others"
# levels(total$Item_Type)[levels(total$Item_Type) %in% c("Seafood","Meat","Canned")]<-"Frozen Foods"
# levels(total$Item_Type)[levels(total$Item_Type) %in% c("Hard Drinks","Soft Drinks")]<-"Drinks"
# 
# 
# ggplot(total,aes(x=Outlet_Identifier,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# levels(total$Outlet_Identifier)[levels(total$Outlet_Identifier) %in% c("OUT010","OUT019")]<-"OUT027"
# 
# 
# ggplot(total,aes(x=Item_Fat_Content,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# ggplot(total,aes(x=Outlet_Size,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# ggplot(total,aes(x=Outlet_Type,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# ggplot(total,aes(x=Outlet_Location_Type,y=Item_Outlet_Sales))+geom_bar(stat="identity")
# 
